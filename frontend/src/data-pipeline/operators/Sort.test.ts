...
describe('SortOperator', () => {
  describe('Basic Sorting', () => {
    ...
    it('should sort string column ascending', () => {
      ...
      const result = SortOperator.apply(table, { column: 'name', order: 'asc' });
    ...
    it('should sort string column descending', () => {
      ...
      const result = SortOperator.apply(table, { column: 'name', order: 'desc' });
      const names = getColumnValues(result, 'name');
  ...
  describe('Top-K Sorting', () => {
    it('should return top k elements in sorted order', () => {
      ...
      const result = SortOperator.applyTopK(table, { column: 'id', order: 'asc' }, 3);
    ...
    it('should handle limit larger than table size', () => {
      ...
      const result = SortOperator.applyTopK(table, { column: 'id', order: 'asc' }, 10);
      const ids = getColumnValues(result, 'id');
    ...
    it('should work with descending order for top-k', () => {
      ...
      const result = SortOperator.applyTopK(table, { column: 'id', order: 'desc' }, 3);
  ...
  describe('Edge Cases', () => {
    ...
    it('should preserve stable sort for equal values', () => {
      ...
      const result = SortOperator.apply(table, { column: 'category', order: 'asc' });
  ...
  describe('Error Handling', () => {
    ...
    it('should throw error for non-existent column in top-k', () => {
      ...
      expect(() => {
        SortOperator.applyTopK(table, { column: 'nonexistent', order: 'asc' }, 2);
      }).toThrow("Column 'nonexistent' not found");
    });
  });
  ...
  describe('Security Tests', () => {
    it('should safely handle column names with SQL injection patterns', () => {
      ...
      expect(() => {
        SortOperator.apply(table, { column: "'; DROP TABLE users; --", order: 'asc' });
    ...
    it('should handle extremely large numbers', () => {
      ...
      expect(ids).toEqual([Number.MIN_SAFE_INTEGER, 0, Number.MAX_SAFE_INTEGER]);
    });
  ...
  describe('Data Type Handling', () => {
    it('should sort floating point numbers correctly', () => {
      ...
      const result = SortOperator.apply(table, { column: 'value', order: 'asc' });
      const values = getColumnValues(result, 'value');
    ...
    it('should handle boolean values', () => {
      ...
      const result = SortOperator.apply(table, { column: 'flag', order: 'asc' });
  ...
  describe('Performance Tests', () => {
    ...
    it('should be efficient with top-k partial sort', () => {
      ...
      const start = performance.now();
      const result = SortOperator.applyTopK(table, { column: 'value', order: 'asc' }, 100);
      const duration = performance.now() - start;
  ...
  describe('Memory Efficiency', () => {
    it('should not leak memory on repeated sorts', () => {
      ...
      // Perform many sorts
      for (let i = 0; i < 100; i++) {
        SortOperator.apply(table, { column: 'value', order: i % 2 === 0 ? 'asc' : 'desc' });
      }
    ...
    it('should handle sorting with many columns efficiently', () => {
      ...
      const start = performance.now();
      const result = SortOperator.apply(table, { column: 'col_0', order: 'asc' });
      const duration = performance.now() - start;
